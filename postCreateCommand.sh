...
export ILIAS_VERSION=8
export ADMIN_NAME=admin
export ADMIN_PASS=admin
export ADMIN_MAIL=admin@idev.dev
export DB_ROOT_PWD=mariadb
export DB_HOST=db
export DB_NAME=ilias
export DB_USER=ilias
export DB_USER_PWD=ilias
...
for ILIAS_VERSION in $ILIAS_VERSION; do
  ...
  sudo echo ${MIN_CONFIG_JSON} >/tmp/minimal-config.json
  sudo cp /tmp/minimal-config.json /var/www/minimal-config.json
...
sudo cp /tmp/php.ini /usr/local/etc/php/php.ini

if [ ! -d "$DATADIR/iliastest" ]; then
  sudo mkdir ${DATADIR}/iliastest
  sudo echo -e "iliastest created"
fi
